...
@app.route("/popular-articles",method="GET")
def popular_articles():
    popular_articles_data=[]
    for index,row in output.iter():
        _p={
            "url": row["url"],
            "title": row["title"],
            "text": row["text"],
...
@app.route("/recommended-articles",method="GET")
def recommended_articles():
    ...
    for liked_article in liked_articles():
        output=get_recommendation(liked_article["title"])
